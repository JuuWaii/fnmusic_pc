...
- 飞牛音乐播放器使用 WebAudio（AudioContext）：客户端对正在播放的音频上下文采用
  「暂停→切换→恢复」序列强制重路由（毫秒级中断，Chromium/Windows 平台的已知限制）。
- 纯 HTTP 的内网地址在 Chromium 中属于非安全上下文，客户端启动时会自动将你配置的
  服务器地址标记为安全上下文以启用设备枚举；**更换服务器地址后需重启客户端生效**。

## 🖥 显示兼容性

- 默认禁用硬件加速（软件渲染）：部分显卡/驱动/远程桌面环境下 Chromium GPU 合成会失败，
...
- 点击主窗口关闭按钮（X）默认**最小化到系统托盘**，音乐在后台继续播放（可在
  设置 → 高级 中关闭该行为）；
- 托盘图标左键单击：显示/隐藏主窗口；右键菜单：显示、设置、桌面歌词开关、退出；
- 托盘「退出」才会真正结束应用；
