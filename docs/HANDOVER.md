...
- **v0.1.11**（当前）**自动登录**：登录态根因=门户 music-token 为 session cookie（重启即丢）→ 设置页保存账号密码（safeStorage/DPAPI 加密落盘，无明文）→ 登录页自动填写提交（iframe 全注入 + origin 过滤 + React/Vue 兼容 + SPA MutationObserver）；两轮审查处置（welcome 选择器/清空语义/origin 过滤/可解密判定/长度上限）
...
## 6. 当前状态（v0.1.11，工作区干净）

- git：35 提交，HEAD = `d795a5f`（+版本号 0.1.11 待提交）；`git status` 干净
- 测试：`npm test` → scripts/test-headless.js **59/59 通过**
- 隐私：`npm run check:privacy` → 42 文件 0 违规（含 FN Connect 个人路径段检测）
- 产物：`dist\FNMusicPC Setup 0.1.11.exe`（安装版）、`dist\FNMusicPC 0.1.11.exe`（便携版）
- 依赖：electron ^33.4.11、electron-builder ^26.15.3、node_modules 已装（含手动下载的 electron 二进制）

## 7. 待办与验证清单（用户真机）

1. **v0.1.11 验证（重点）**：设置页填账号密码保存 → 重启 → 登录页自动填写提交；凭据 DPAPI 加密仅本机可解密；音频面板枚举正常（iframe 回退）；
