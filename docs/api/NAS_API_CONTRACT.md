...
2026-09-15 专辑与歌手参数、响应消费方式已进一步静态核实并接入原生，详见 [专辑与歌手契约及验收](../NATIVE_COLLECTIONS.md)。2026-09-16 已完成当前实例只读验收：两类列表、详情及详情歌曲首/次页全部通过，确认服务端分页有效。
...
已从缓存前端调用确认 `GET /search/track` 参数为 `q、page、size`，响应 data 包含 `list、total`，曲目使用与曲库相同的映射。原生已实现并通过合成契约测试；真实 NAS 搜索未执行，验证状态见 [搜索验收](../NATIVE_SEARCH.md)。

## 2026-09-15 搜索真实验证（优先于前文静态状态）

使用本应用已保存会话、无 authx，只读搜索已成功。当前实例忽略 search/track 的 page/size：短前缀多结果查询时，第一与第二页请求均返回同一个完整 list，list 条数等于 total；未观察到服务端分页。

原生对可确认的完整集合做本地分页，正常部分页保持原样。实测修复后的两页各一条且不同，总数一致；精确标题命中及随机无匹配查询均通过。不得将当前实例的全量行为泛化为其他服务端版本；响应上限与拒绝异常结构的边界仍有效。
