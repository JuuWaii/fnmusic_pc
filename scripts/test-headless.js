...
console.log('\n[1b] autoLoginSnippet 按钮匹配（审查轮 12：不再误点「使用 NAS 登录」）');
{
  ...
  const m = src.match(/function autoLoginSnippet\(username, password, trustedOrigins, isMain\) \{[\s\S]*?\n  \}/);
  assert.ok(m, 'autoLoginSnippet 应存在于 window-manager.js');
  const code = new Function('username', 'password', 'trustedOrigins', 'isMain', m[0] + '\nreturn autoLoginSnippet;')()('u', 'p', [], false);
